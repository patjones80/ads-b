...
with open("collected_history.json", mode="r", encoding="utf-8") as history:
    ...
    # The dates and times are coming in as strings so convert them back
    for ix, row in enumerate(data):
        adsb_date, adsb_time = row['adsb_date'], row['adsb_time']

        data[ix]['adsb_date'] = datetime.strptime(adsb_date, '%Y-%m-%d')
        data[ix]['adsb_time'] = datetime.strptime(adsb_time, '%H:%M:%S')

    # We take the combination of "now" and "hex" to be a compound key
